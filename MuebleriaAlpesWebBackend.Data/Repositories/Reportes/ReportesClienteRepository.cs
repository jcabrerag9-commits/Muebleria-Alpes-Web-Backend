...
namespace MuebleriaAlpesWebBackend.Data.Repositories.Reportes
{
    public class ReportesClienteRepository : IReportesClienteRepository
    {
        ...
        {
            var resultado = new List<FiltroClienteResponse>();
            ...
            const string sql = @"
                SELECT CLI_CLIENTE,
                       CLI_CODIGO,
                       TRIM(CLI_PRIMER_NOMBRE || ' ' || NVL(CLI_SEGUNDO_NOMBRE, '') || ' ' || CLI_PRIMER_APELLIDO || ' ' || NVL(CLI_SEGUNDO_APELLIDO, '')) AS CLIENTE_NOMBRE,
                       CLI_NUMERO_DOCUMENTO,
                       CLI_ESTADO
                  FROM ALP_CLIENTE
                 WHERE CLI_ESTADO = 'ACTIVO'
                 ORDER BY CLI_CLIENTE DESC";
            ...
            while (await reader.ReadAsync())
            {
                resultado.Add(new FiltroClienteResponse
                {
                    ClienteId = ConvertirInt(reader["CLI_CLIENTE"]),
                    Codigo = ConvertirString(reader["CLI_CODIGO"]),
                    Nombre = ConvertirString(reader["CLIENTE_NOMBRE"]),
                    Documento = ConvertirString(reader["CLI_NUMERO_DOCUMENTO"]),
                    Estado = ConvertirString(reader["CLI_ESTADO"])
                });
            }
