...
namespace MuebleriaAlpesWebBackend.Data.Repositories.Reportes
{
    public class ReportesVentasRepository : IReportesVentasRepository
    {
        ...
        public async Task<List<FiltroCanalVentaResponse>> ListarCanalesVentaFiltroAsync()
        {
            var resultado = new List<FiltroCanalVentaResponse>();

            using var connection = (OracleConnection)_connectionFactory.CreateConnection();
            await connection.OpenAsync();

            const string sql = @"
                SELECT CVE_CANAL_VENTA,
                       CVE_CODIGO,
                       CVE_NOMBRE,
                       CVE_DESCRIPCION,
                       'ACTIVO' AS CVE_ESTADO
                  FROM ALP_CANAL_VENTA
                 ORDER BY CVE_CANAL_VENTA DESC";
            ...
            while (await reader.ReadAsync())
            {
                resultado.Add(new FiltroCanalVentaResponse
                {
                    CanalVentaId = ConvertirInt(reader["CVE_CANAL_VENTA"]),
                    Codigo = ConvertirString(reader["CVE_CODIGO"]),
                    Nombre = ConvertirString(reader["CVE_NOMBRE"]),
                    Descripcion = ConvertirString(reader["CVE_DESCRIPCION"]),
                    Estado = ConvertirString(reader["CVE_ESTADO"])
                });
            }
        ...
        public async Task<List<FiltroCiudadResponse>> ListarCiudadesFiltroAsync()
        {
            var resultado = new List<FiltroCiudadResponse>();
            ...
            const string sql = @"
                SELECT CIU_CIUDAD,
                       ...
                       CIU_ESTADO
                  FROM ALP_CIUDAD
                 WHERE CIU_ESTADO = 'ACTIVO'
                 ORDER BY CIU_CIUDAD DESC";
            ...
            while (await reader.ReadAsync())
            {
                resultado.Add(new FiltroCiudadResponse
                {
                    CiudadId = ConvertirInt(reader["CIU_CIUDAD"]),
                    Codigo = ConvertirString(reader["CIU_CODIGO"]),
                    Nombre = ConvertirString(reader["CIU_NOMBRE"]),
                    DepartamentoId = ConvertirInt(reader["DEP_DEPARTAMENTO"]),
                    Estado = ConvertirString(reader["CIU_ESTADO"])
                });
            }
