...
namespace MuebleriaAlpesWebBackend.API.Controllers.Reportes
{
    ...
    public class ReportesCajaController : ControllerBase
    {
        ...
        [HttpGet("filtros/cortes-caja")]
        public async Task<IActionResult> ListarCortesCajaFiltro([FromQuery] string? estado)
        {
            try
            {
                var response = await _reportesCajaService.ListarCortesCajaFiltroAsync(estado);
                return Ok(new { success = true, data = response });
            }
            catch (Exception ex)
            {
